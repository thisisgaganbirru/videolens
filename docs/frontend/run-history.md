...
- 2026-09-17 · main session · `FetchRunsGateway` now sits on the shared `infrastructure/apiClient.ts` (adds `Authorization: Bearer` when signed in); `ApiError` carries `code` and `status`; for workspace callers history merges Redis and the Postgres archive server-side
