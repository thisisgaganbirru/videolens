...
## Changelog
- 2026-09-17 · main session · `RunStore` now stores `workspace_id`, `plan`, `max_duration_seconds`, `source_url`, `duration_seconds`, `usage` and gained `set_duration`/`set_usage`; the container wraps it in `TieredRunRepository` (Postgres archive for workspace runs, pass-through without `DATABASE_URL`) — see `library-search.md`
